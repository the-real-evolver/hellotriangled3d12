...
static const char* const WindowTitle = "Hello Triangle D3D12 (C) 2018 Christian Bleicher";
...
*/
LRESULT WINAPI
WindowCallback(HWND wnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    switch (msg)
    {
        case WM_DESTROY:
...
*/
int
WinMain(_In_ HINSTANCE instance, _In_opt_ HINSTANCE /*hPrevInstance*/, _In_ LPSTR , _In_ int)
{
    ID3D12Device* device;
    ...
    /* create render target views and add them to the descriptor heap */
    ...
    result = D3DCompile(DefaultGpuProgram, strlen(DefaultGpuProgram), "", NULL, NULL, "DefaultVertexShader", "vs_5_0", 0U, 0U, &vertexShader, NULL);
    ID3DBlob* pixelShader;
    result = D3DCompile(DefaultGpuProgram, strlen(DefaultGpuProgram), "", NULL, NULL, "DefaultFragmentShader", "ps_5_0", 0U, 0U, &pixelShader, NULL);
    ...
    /* create upload buffer on the cpu and copy vertex data into it */
    struct Vertex
    {
        float position[3U];
        float uv[2U];
    ...
    /* create vertex buffer on the gpu and copy data from upload buffer */
    ...
    /* create allocators and commandlists */
    for (i = 0U; i < BackBufferCount; ++i)
    {
    ...
    while (msg.message != WM_QUIT)
    {
        ...
        static const float ClearColor[] = {0.2f, 0.2666f, 0.3333f, 1.0f};
